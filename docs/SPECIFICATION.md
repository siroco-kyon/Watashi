...
(例: 最終ローカルパス `LastLocalPath`)。設定単位はWindowsユーザー + `BrandId` であり、
同じWindowsユーザーがWatashi内でログインし直しても共有される。
...
旧 `settings.json` に新しいキーがない場合は、従来互換の「ローカルは前回場所、リモートは未選択、
並び順は記憶しない」を既定値とする。固定ローカル場所を開けない場合は `LastLocalPath`、
Windowsユーザーフォルダの順にフォールバックする。保存済みリモート場所は起動時の権限カタログと
PermissionId / host / share / 許可ルートを照合し、権限外になった値を開始入口として使用しない。
...
転送センターは非モーダルで、状態フィルター・転送元／先・サイズ／日時・再試行／エラー詳細を表示する。転送センターは一覧を広く表示し、「詳細を開く」で転送元・先とエラーを確認します。高さが足りない画面では「一覧へ戻る」で切り替え、広い画面では境界をドラッグして一覧と詳細の高さを調整できます。画面全体と詳細はスクロールでき、長いパスやエラー本文は選択してコピーできます。状態フィルターには件数を表示し、「要対応」から失敗・一時停止・競合待ちを確認できます。「完了・スキップの履歴を消去」は再開情報を残して整理します。「失敗・中止の履歴を破棄」は確認後に対象の履歴と途中データを消去し、再開できなくなります。「すべて中止」と履歴整理は絞り込みに関係なく対象件数を確認し、確認時に対象だった転送のみを処理します。conflict_waitingは上書き・スキップ・別名の判断を待つ。期限切れuploadは失敗とし、利用者の再試行で先頭から再送する。
...
- .NET 10 WPF `ThemeMode` を基盤に、メイン画面の月／太陽ボタンで Light / Dark を即時切替して利用者設定へ保存。ログイン画面は保存済みテーマを適用するが切替操作は置かない。旧 System 設定は更新時に現在の見た目へ一度だけ移行
...
- 右パネル: 「付与済みのパス」「＋ パスを追加」「✏ 選択を編集」「🔎 実効権限プレビュー」を切り替えて表示。同じユーザー内の切替では入力と選択を保持
- 権限セットも「内容一覧」「＋ 行を追加」で表示領域を共用し、行の追加後は上部の「保存」で確定
- 両画面のフォルダー一覧は高さ200 DIP。「広く開く」は元のViewModelを共有するサイズ変更可能な選択ウィンドウで、選択は許可パスの入力に反映し、サーバーへの保存は行わない
- 管理ウィンドウは初期1220×800 DIP、最小720×420 DIP。開くときに所有者のモニターの作業領域とDPIに合わせて初期サイズ・最小サイズを上限調整
- 本文は最低900×600 DIPを確保し、小さい画面では本文全体を縦横スクロール。一覧の高さは有限に保ち、仮想化を維持。ホスト・共有・テンプレート・システム設定の編集フォームも縦スクロール可能
