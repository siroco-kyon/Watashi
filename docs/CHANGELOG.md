...
- クライアントに**表示テーマ**を追加。ヘッダーの ☀ / ☾ ボタンから「OS の設定に従う」(既定) / 「ライト」/ 「ダーク」を選択でき、選択は端末ごとに `settings.json` (`Theme`) へ保存される
- 「OS の設定に従う」は Windows の「アプリのモード」(`AppsUseLightTheme`) を参照し、`SystemEvents.UserPreferenceChanged` で OS 側の変更にも追従する
- タイトルバー (非クライアント領域) も `DWMWA_USE_IMMERSIVE_DARK_MODE` でダーク化。未対応の Windows では本文のみダークになり、例外は出さない
- 配色を `Themes/Colors.Light.xaml` / `Themes/Colors.Dark.xaml` に分離。`Themes/Colors.xaml` はブラシ定義だけを持ち、色は `DynamicResource` で引く。辞書を差し替えてもブラシのインスタンスは変わらないため、開いたままのウィンドウも**再起動なしで**切り替わる
- WPF 既定テンプレートが配色をハードコードしている `ContextMenu` / `MenuItem` / `Separator` / `ToolTip` にテンプレートを追加し、ポップアップもテーマに追従させた

### UX 改善

- ダーク配色は明度の単純反転ではなく、暗い面での可読性を基準に選び直した。文字は WCAG 2.1 AA (4.5:1)、枠線・フォーカスリングは 3:1 を満たす
- 鳥居の朱色は色相・彩度を保ったまま明度のみ調整。ダークでは朱の上に載る文字色 (`TextOnAccentColor`) を白から濃色へ反転させ、ボタン文字のコントラストを確保している

### 修正

- なし (既存動作の変更なし。ライトテーマの色は従来と同一値)

### 既知の制限

- 確認ダイアログ (`MessageBox`) とファイル選択ダイアログは Windows ネイティブの部品のため、アプリ側の設定ではなく OS のテーマに従う
- Windows のハイコントラストモード使用時は OS 側を優先し、ダーク配色を適用しない (`Controls.xaml` の各スタイルがシステム色へ切り替わる既存の仕組みを壊さないため)

### 文書

- `docs/USER-GUIDE.md` に「表示テーマ (ダークモード)」節を追加し、ヘッダー図を更新
- `docs/BRANDING.md` §5-4 を、配色が 2 ファイルに分かれた構成へ更新。ブランドカラー変更時の注意 (両ファイルのキー一致・`.ico` 生成スクリプトとの同期・コントラストテスト) を明記
- `docs/FEATURES.md` / `docs/DEVELOPMENT.md` のファイル構成を更新
- `Watashi_手動検証項目リスト.csv` に表示テーマの検証項目を追加
