...
## 2026-09-11 — PoC改善報告に利用者の具体的な声を反映

- 種類別の色分け、拡張子順の並べ替え、複数ファイルのドラッグ＆ドロップ転送、1画面ずつのスクロール、スクロールバーと画面の見やすさ、500件表示時の追加読み込みに関する声を反映。重複する表現は差し替え、その他の聞き取り内容は維持。
- 見た目と大量ファイル操作の見出しを調整し、種類別アイコンと任意のファイル名色分けを区別。2000件への読み込み単位拡大は手間の軽減であり、全件自動表示ではないことを明記。
- 改善件数28件と全体展開の日程は変更なし。文書のみの変更で、アプリの再発行・配布は不要。

## 2026-09-11 — PoC改善報告の全体展開予定を更新

- 報告資料からPR番号と改修との対応付け、および「資料と実装のズレを止める」の項目を削除。改善内容は28件（利用者18件・管理者10件）として図・集計・付録を同期。
- サーバー更新済み、クライアントは起動時に自動更新される現状を反映。9月14日から全体展開、9月14日〜27日は約2週間の初期流動、9月28日以降を通常運用移行の目安として掲載。
- PoCの改善サイクル図の戻り矢印を枠の外側へ移し、管理者の枠との重なりを解消。
- 文書のみの変更。アプリの再発行・配布は不要。

## 2026-09-10 — PoC改善報告を現行仕様へ更新

- PoC改善報告に一覧の連続キー操作・フォーカス維持・削除成功通知の省略（#152・#153・#156）を反映。掲載PRは30件（利用者側18件・管理者側12件）へ更新し、図と付録も同期。
- 2000件は初回・追加の読み込み単位であること、履歴破棄時の途中データ削除、メンテナンス待機の再開条件と配布手順を明確化。
- 過去の227項目の手動検証記録と、現在の自動テスト・WPF描画・キー入力検証を区別。文書更新やPRマージを配布・受入確認の完了として扱わない説明へ修正。
- 文書のみの変更。アプリの再発行・配布は不要。

## 2026-09-10 — ファイル操作後のキーボード操作を継続

- Backspace／Alt+↑、履歴移動、F5、パス入力・移動ボタンを共通の選択・フォーカス復元へ統合。親へ戻る場合は元のフォルダを選択し、更新と履歴では選択・スクロールを復元。
- F6と絞り込み解除、削除・名前変更・新規フォルダ作成・追加読込後も一覧で操作を継続。入力先を変えた場合や古い操作の完了ではフォーカスを奪わない。
- 両ペイン更新は両方の完了を待機。失敗して一覧を操作できない場合はパス欄へ戻す。
- 実際のWPFフォーカスと連続キー操作を検証する `--keyboard-input` テストを追加。読み込み中にBackspace・PageDownなどを再度押しても、完了後の操作継続を妨げないことを確認。

## 2026-09-09 — 技術レポートを現行仕様へ更新

- 技術レポートの転送・一覧・キーボード操作・管理画面・メンテナンス・共有廃止の説明を現行仕様へ同期
- 構成図、操作フロー、初回設定フローと画面差し替え枠の文字サイズ・余白を調整し、SVGとPNGを共通定義から生成。Wordも再生成
- 過去の手動検証記録と今回の841件の自動回帰検証・WPFレイアウト検証を区別。文書のみの変更でアプリの再配布は不要

## 2026-09-09 — 読み込み中のショートカットによるフォーカス移動を維持

- フォルダ読み込み中に F6／Shift+F6、Ctrl+L、Ctrl+F を使った場合、読み込み完了後に一覧へフォーカスを奪い返さないよう修正。先に処理されたキーイベントも検知します。
- WPFの回帰テストに処理済みイベントによる復元中止、上下キー・Enterによる操作継続、監視解除を追加。

## 2026-09-09 — フォルダ移動後のキーボード操作と削除通知を修正
...
## 2026-09-06 — 試験導入（PoC）改善結果の報告資料を追加

- [PoC 改善結果 報告資料](poc-improvement-report/index.html)を追加。Pull Request #118〜#150 の改修 27 件を「利用者からの要望（15件）」と「管理者として把握・改善した点（12件）」に整理し、7 点の SVG 図で説明
- 改善サイクル、起点別の対応件数、ファイル一覧・ホーム画面・個人設定のビフォー／アフター、自動テスト件数の推移（613→841件）、本格導入のロードマップを掲載
- スクリーンショット差し替え枠を 3 箇所用意（`docs/poc-improvement-report/images/` に配置すると自動表示）
- 文書のみの追加。アプリの再配布は不要

## 2026-09-06 — HTML版の画面構成・ブランディング案内を同期

- 利用者HTMLマニュアルの状態バー説明と構成図を実装に合わせ、転送進捗・待機／要対応件数とヘッダーの利用者名を案内
- ブランディングHTML版にバージョン情報・初回パスワード・メンテナンス画面を追記。コピー内容、鳥居の文言・読み上げ名の変更範囲を明記し、Markdown版も同期
- 文書のみの修正。アプリの再配布は不要

## 2026-09-06 — ホーム下部にコンパクトな転送状況を追加
...
- 既存の下部ステータス欄を使用し、接続状態と操作結果も保持。利用者名はヘッダーに表示
- ライト／ダーク、幅に応じた表示、2件の並列進捗、更新・中止・完了、センターへの操作をWPFで検証
- Clientの再発行・配布が必要。API・DB・同時転送数の変更なし
