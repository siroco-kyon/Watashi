...
def rounded_box(draw, xy, fill, outline, title, subtitle=None, title_size=32):
    ...
    if subtitle:
        ...
        tx = (x1 + x2 - (bbox[2] - bbox[0])) / 2
        draw.multiline_text((tx, ty + 48), subtitle, font=sub_font, fill="#475569", spacing=5, align="center")


def arrow(draw, start, end, label=None, color="#2E74B5"):
...
def build_diagrams(asset_dir: Path) -> dict[str, tuple[Path, Path]]:
    ...
    draw.text((70, 60), "Watashiの構成と信頼境界", font=font(40), fill="#17324D")
    ...
    draw = ImageDraw.Draw(image)
    draw.text((65, 42), "ファイル操作の共通処理", font=font(40), fill="#17324D")
    labels = [
        ("1", "認証", "JWT・利用者状態"),
        ("2", "パス解決", "正規化・境界"),
        ("3", "認可", "共有×サブパス×操作"),
    ...
    x = 55
    for idx, (num, title, subtitle) in enumerate(labels):
        w = 245
        fill = "#EAF2F8" if idx < 3 else "#E8F5F2"
        outline = "#2E74B5" if idx < 3 else "#2A8C82"
        rounded_box(draw, (x, 190, x+w, 455), fill, outline, f"{num}. {title}", subtitle, title_size=27)
        if idx < len(labels) - 1:
...
<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="1050" viewBox="0 0 1800 1050" role="img" aria-labelledby="title desc">
  ...
  <defs>
    ...
    <style>
      .title{font:700 40px 'Yu Gothic',sans-serif;fill:#17324D}.box-title{font:700 32px 'Yu Gothic',sans-serif;fill:#17324D;text-anchor:middle}.sub{font:400 20px 'Yu Gothic',sans-serif;fill:#475569;text-anchor:middle}.label{font:600 19px 'Yu Gothic',sans-serif;fill:#334155;text-anchor:middle}.note{font:400 20px 'Yu Gothic',sans-serif;fill:#475569}
    </style>
  </defs>
...
<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="620" viewBox="0 0 1800 620" role="img" aria-labelledby="title desc">
  ...
  <defs><marker id="arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0 0 L10 5 L0 10z" fill="#64748B"/></marker><style>.title{font:700 40px 'Yu Gothic',sans-serif;fill:#17324D}.step{font:700 27px 'Yu Gothic',sans-serif;fill:#17324D;text-anchor:middle}.sub{font:400 19px 'Yu Gothic',sans-serif;fill:#475569;text-anchor:middle}.note{font:400 22px 'Yu Gothic',sans-serif;fill:#475569}</style></defs>
  <text x="65" y="82" class="title">ファイル操作の共通処理</text>
  <g>
    <g><rect x="55" y="190" width="245" height="265" rx="22" fill="#EAF2F8" stroke="#2E74B5" stroke-width="4"/><text x="177.5" y="280" class="step">1. 認証</text><text x="177.5" y="340" class="sub">JWT・利用者状態</text></g>
    <g><rect x="350" y="190" width="245" height="265" rx="22" fill="#EAF2F8" stroke="#2E74B5" stroke-width="4"/><text x="472.5" y="280" class="step">2. パス解決</text><text x="472.5" y="340" class="sub">正規化・境界</text></g>
    <g><rect x="645" y="190" width="245" height="265" rx="22" fill="#EAF2F8" stroke="#2E74B5" stroke-width="4"/><text x="767.5" y="280" class="step">3. 認可</text><text x="767.5" y="340" class="sub">共有×サブパス×操作</text></g>
    <g><rect x="940" y="190" width="245" height="265" rx="22" fill="#E8F5F2" stroke="#2A8C82" stroke-width="4"/><text x="1062.5" y="280" class="step">4. 経路選択</text><text x="1062.5" y="340" class="sub">Direct / Agent</text></g>
    <g><rect x="1235" y="190" width="245" height="265" rx="22" fill="#E8F5F2" stroke="#2A8C82" stroke-width="4"/><text x="1357.5" y="280" class="step">5. 実行</text><text x="1357.5" y="340" class="sub">SMB操作</text></g>
...
def build_screenshot_placeholders(asset_dir: Path) -> dict[str, tuple[Path, Path]]:
    ...
    for key, spec in SCREENSHOT_SPECS.items():
        ...
        draw.text(((1800 - (title_box[2] - title_box[0])) / 2, 390), spec["title"], font=title_font, fill="#17324D")
        guide_box = draw.multiline_textbbox((0, 0), spec["guide"], font=guide_font, spacing=14, align="center")
        draw.multiline_text(((1800 - (guide_box[2] - guide_box[0])) / 2, 485), spec["guide"], font=guide_font, fill="#475569", spacing=14, align="center")
        image.save(png, dpi=(180, 180))

        guide_lines = spec["guide"].split("\n")
        tspans = "".join(f'<tspan x="900" dy="42">{escape(line)}</tspan>' for line in guide_lines)
        svg.write_text(f"""
<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="1012" viewBox="0 0 1800 1012" role="img" aria-label="{escape(spec['alt'])}">
  ...
  <text x="900" y="435" text-anchor="middle" font-family="Yu Gothic, sans-serif" font-size="42" font-weight="700" fill="#17324D">{escape(spec['title'])}</text>
  <text x="900" y="500" text-anchor="middle" font-family="Yu Gothic, sans-serif" font-size="26" fill="#475569">{tspans}</text>
</svg>
""".strip(), encoding="utf-8")
        results[key] = (png, svg)
    return results
...
def add_picture_with_alt(doc, path: Path, width: float, alt_text: str, svg_path: Path | None = None) -> None:
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def render_markdown(doc: Document, lines: list[str], assets: dict[str, tuple[Path, Path]], main_body=False) -> None:
    ...
    while i < len(lines):
        ...
        if screenshot:
            ...
            add_picture_with_alt(doc, png, 6.15, spec["alt"], svg)
