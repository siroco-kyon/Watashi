...
7. ユーザー権限割当 (ユーザー × 共有 × サブパス × テンプレート)
       ↓
8. ユーザーに通知 (URL のみ。初期パスワードの配布は不要)
```
...
> 💡 **初期パスワードの配布は不要です。** ユーザーを作成すると「初回設定待ち」の状態になり、本人が初回ログイン時に自分でパスワードを決めます。本人確認は Windows 統合認証 (ログオン中の Windows ユーザー名 = GID = Watashi ユーザー名) で行われるため、管理者がパスワードを決めて伝える手順そのものがなくなります。
>
