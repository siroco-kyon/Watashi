...
3. AdminWindow が11個のタブで開く

管理画面は開くときにモニターの作業領域と表示倍率に合わせてサイズを調整します。小さい画面では本文全体を縦・横にスクロールできます。一覧は一覧内で、長い編集フォームはフォーム内でスクロールします。左右の境界をドラッグして幅も調整できます。
...
任意の「名前」を登録すると、管理画面のユーザー管理・ユーザー権限・信頼デバイス・操作ログで `山田 太郎（G012345）` のように表示されます。名前は最大100文字で、空欄でも構いません。名前は表示と検索のためだけの項目で、ログインには引き続きユーザー名 (GID) を使います。
...
- 必須列: `Username`
- 任意列: `IsAdmin` (新規作成時は省略すると false)、`DisplayName` (最大100文字)
- 列順は問わない (ヘッダー名で判定)
...
| 上書き | 既存ユーザーの `IsAdmin` と、CSVに列がある場合の `DisplayName` を更新する。`DisplayName` の明示的な空セルは名前を消去し、列や行末セル自体がなければ現在名を維持する。**パスワードには一切触れない**。 |
...
既存の列順を維持し、末尾に `DisplayName` が追加される。
...
**運用 tips**:
- 退職者: **無効化**を使用。理由・日時・操作者を残し、すべてのaccess/refresh tokenと信頼端末を即時失効しつつ、ユーザーID・権限・過去監査ログを維持する。復帰時は明示的に再有効化する
...
共有には未完了の転送 (アップロードセッション) とごみ箱台帳が紐づきます。これらが残っている間は、
**ホスト・共有名の変更と共有の削除がブロックされます** (409 `share_has_durable_state`)。
台帳は現在の接続先のパスを指しているため、実体を回収しないまま付け替えると、
後片付けが新しいストレージ上の無関係なファイルを消しに行くおそれがあるからです。

共有を選択すると詳細ペインに「**未完了の転送・ごみ箱データ**」が表示されます。

| 表示 | 意味 | 対応 |
|---|---|---|
| 残存 0 件 | ブロックなし | そのまま変更・削除できる |
| 残存 n 件 (回収失敗 0 件) | 進行中または期限待ち | **待てば自動的に解消**する (転送は最終通信から24時間 + 掃引15分、ごみ箱は保持期間経過後) |
| 回収失敗 n 件 (`cleanup_retry` / `purge_retry`) | 実体を削除できずに再試行中 | **待っても解消しない。**共有へ到達できない状態なので「強制解除」を使う |

**強制解除** ボタンは、まず正規の後片付け (一時ファイルの削除) を試み、到達できないものだけ台帳を
終端させます。したがって共有が生きていれば実体ごと回収され、死んでいる場合だけ台帳を諦めます。

- 進行中の転送があれば中断されます
- 回収できなかった実体は共有上にゴミとして残ります。ストレージが復旧したら手動で削除してください
- 実施内容と回収できなかったパスは監査ログ `ADMIN_SHARE_RELEASE_DURABLE` に記録されます

計画的に廃止できる場合は、**旧ストレージが生きているうちに残存 0 件を確認してから**変更・削除するのが
最も安全です。強制解除は、既に到達できなくなった共有を片付けるための手段です。

なお回収失敗が続く台帳は、転送は作成から `TransferV2:CleanupGiveUpDays` (既定 7 日) + セッション有効期間、
ごみ箱は保持期間 + 7 日を過ぎると自動的に諦めて終端します (`cleanup_gave_up`)。
放置しても最終的にブロックは解けますが、それまで死んだホストへの接続試行と警告ログが続きます。
...
左サイドバーは `名前（Username）` で表示され、名前と Username のどちらでも絞り込めます。
右側は **「付与済みのパス」「＋ パスを追加」「✏ 選択を編集」「🔎 実効権限プレビュー」** を切り替えて使います。一覧と長いフォームを同時に上下へ詰め込まず、それぞれに表示領域を確保します。同じユーザー内でこの切替をしても、入力途中の値と選択行は保持されます。別のユーザーへの切替や再読み込み、保存まで保証するものではありません。

```
ユーザー一覧  │ 選択ユーザー・処理結果
              │ [付与済みのパス] [＋ パスを追加] [✏ 選択を編集] [🔎 実効権限プレビュー]
              │
              │ 選んだ一覧またはフォームを広く表示
              │ （長いフォームは最後までスクロール）
```
...
3. (任意) **ブラウズ** の「開く」で実在ディレクトリを取得し、サブフォルダーをクリックすると許可パスに反映。高さ200の一覧で足りない場合は **「広く開く」** でサイズ変更可能な別ウィンドウを使えます。参照パス・一覧・選択は元のフォームと共有され、「閉じて戻る」では権限を保存しません。
4. 必要に応じて有効開始・有効期限（端末のローカル日時）、理由・チケット番号を入力し、フォーム末尾の **「＋ この期限付き権限を追加」** を押す。有効開始・期限は空欄なら制限なし。
...
右側の **「内容一覧」「＋ 行を追加」** を切り替えて使います。セット名・説明と上部の「保存」は共通です。「＋ 行を追加」のブラウズでも高さ200のフォルダー一覧と「広く開く」が使えます。タブの切替では入力と未保存の行を保持します。


**ワークフロー**:
1. **[+ 新規セット]** で名前を入力 → 空のセットが作成される
2. 左でセットを選択して右に詳細表示
3. **「＋ 行を追加」タブ** で共有 / テンプレート / 許可パス / 表示名を入力 → フォーム末尾の **「＋ 行を追加 (保存はセット側で確定)」** を押す。「内容一覧」で追加した行を確認する。
...
ユーザーの自動ログインを管理。
ユーザー列は `名前（Username）` で表示され、名前・Username・端末・Windowsユーザーを検索できます。
...
- 1ユーザーあたり既定3台（`TrustedDeviceLimit` で1〜20台）。同じ端末の再登録だけ旧トークンをローテーション
- 利用者はメイン画面の「信頼端末」から自分の端末一覧を確認し、不要な端末を個別失効可能
...
**フィルタ**:
- User: 名前・ユーザー名の部分一致、またはユーザーID
- Op: ファイル操作（`READ` / `WRITE` / `DELETE` / `RENAME`）または管理者操作（`ADMIN_*`）
- From / To: UTC 期間指定
- 検索ボタンでクエリ実行

**カラム**:
- Timestamp / `名前（Username）` / Op / Result / **ホスト / 共有** / Path / Bytes / Dur(ms) / Err
- `Username` はイベント発生時の証跡を保持し、現在も存在するユーザーだけ現在の名前を補助表示する。削除済みユーザーは Username のみ
...
- 既存の列位置を維持し、末尾に現在の `DisplayName` を追加
...
[図解付きの導入・更新・復旧手順](../deploy/MAINTENANCE-ROLLOUT-PLAN.html) と、同梱の `deploy/maintenance/Install-StatusAssets.ps1`、`Publish-MaintenanceState.ps1` を参照してください。本番のIIS・DNS・証明書・ACLは環境に合わせた設定が必要です。
...
- **Agent → Server (`/api/internal/*`)**: Server 側で `Agent` 認可ポリシーが要求
  ...
  - 複数 Agent でも同じ共有秘密を利用可能。heartbeat は本文と `X-Watashi-Agent-Id` の AgentId を中央の ExecutionNode 名と照合する
  - 現行 Agent の監査ログは AgentId 別に記録する。旧 Agent のログは互換性のため `(agent:shared-secret)` として受理する
