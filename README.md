...
Ctrl+D（ダウンロード）／Ctrl+U（アップロード）、操作中ペイン・選択件数・転送先の表示、非モーダルの転送センターに対応しています。ホーム下部の1行で実行中の転送の進捗バー・割合・待機件数を確認し、クリックで詳細を開けます。バージョン情報の専用画面では、問い合わせ用のバージョンコピーと、鳥居からのひとことを利用できます。一覧を優先した表示・小画面での詳細切替・全体スクロール・件数付き状態フィルターを利用できます。リモート一覧は初回・追加とも2000件です。Serverを先に更新してからClientを配布してください。
...
- **小さい画面での管理操作**: 本文全体と長いフォームのスクロール、権限一覧／編集の切替、フォルダー選択の「広く開く」に対応
...
実際のフォーカスと連続キー入力の確認方法は[開発者ガイド](docs/DEVELOPMENT.md#キーボード連続操作の検証)を参照してください。
